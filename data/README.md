...
滚动单月主结果中，XGBoost 的全局 WMAPE 为 31.34%，沿用上月销量的朴素基准为 40.99%，相对降低 9.65 个百分点（约 23.5%）。固定六个月压力测试中，最近 6 个月均值为 69.31%，口碑＋冷启动综合方案为 39.07%，相对减少 43.6% 的绝对误差；两种协议对应不同应用场景，指标在各自协议内分别报告。

固定压力测试中的口碑增强点估计改善 0.884 个百分点，5,000 次车系聚类 Bootstrap 的 95% 区间为 −0.284 至 2.108 个百分点，稳定增益证据不足，因此定位为辅助特征。冷启动统计策略覆盖 9 个历史不足车系，用于边界样本处理，不改变 371 个车系的滚动主模型。
