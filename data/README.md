...
当前主实验采用滚动单月协议：每个测试月预测下一个月，并使用当时已经公布的上月真实销量；模型参数在六个月评估窗内锁定。固定起点协议仍保留为压力测试：从 2026 年 1 月一次性递归预测六个月，后续月份只能使用此前预测出来的销量滞后，不能读取测试期真实销量。两种协议回答不同问题，指标不直接横向排名。
...
第二阶段对 9 个“2024 年高销量、2026 测试期全零”车系逐一分型。Model Y 和 Model 3 被确认存在同源快照异常：同源年度表补齐 2022 年 5 行和 2024-06 至 2025-12 的 38 行，并纠正 2022 年 4 个非零错值；同源月页补齐 2026 年 1—6 月 12 行。两款车修复后的 2022、2025 全年合计均与来源页年度总量一致。iCAR 03 的测试期零值由同源停售状态支持。其余 6 个车系仍保持零值并进入人工核验，未自动推断或跨车系合并。原始 `monthly_sales.csv` 不被覆盖；修复函数还会校验原值和唯一键，不符合即停止。

Model Y 和 Model 3 的已知历史缺口已经补齐，覆盖层经过唯一键、来源和年度合计检查后纳入当前可复现面板。高优先级外部核验队列仍保留为审计事项；它不改变已经锁定的测试窗口和结果口径。数据修复后的固定压力测试综合方案为 39.07%，滚动单月主结果为 31.34%，均由保存的产物重新计算得到。

第三阶段对全部 54,918 行、1,017 个车系进行了本地规则扫描。硬结构检查未发现重复车系月、负销量、名称—ID 多对多或缺失月份，但这不等于来源数据正确：年度累计、品牌月销量、品牌有销量车系数、零销量标签和排名是否存在，均与 `monthly_sales` 完全机械一致，不能作为独立交叉验证。

第四阶段使用 `scripts/42_recrawl_pcauto_sales.py` 按源站唯一 ID 批量回采。每年只请求 6 月、12 月两个半年锚点，原始 HTML 缓存在 Git 忽略目录，采集结果不会覆盖原表。首批 10 个高优先级车系共请求 90 页，全部成功并解析 540 个车系月：186 条与原表完全一致，353 条源站显示“—”且原表同样为零，唯一明确差异为五菱宏光S 2026-06，源站为 2,215、原表为零。该行经车系 ID、页面名称和厂商汇总页复核后加入覆盖层。

第五阶段使用 `scripts/43_crosscheck_alternative_sales.py` 对可稳定读取的替代站做显式映射。页面必须同时通过车系名和品牌/厂商检查，再用已修复的 Model Y、Model 3 共 72 个重叠月份校准口径；72 个月全部一致。首批 9 个映射页面解析 324 个逐月值，名爵HS 2022-09 和威马W6 2022-09 分别发现 16 辆、4 辆偏差，并由第二个独立来源复核后加入覆盖层。长安CS55PLUS 虽然后续月份完全一致，但替代页在换代边界与原表不连续，因此保留原值，证明名称相同也不能跳过生命周期校验。当前覆盖层共修复 62 行、涉及 5 个车系，净恢复 1,357,467 辆；外部队列剩余 21 个高优先级和 33 个中优先级车系。

源站“—”不自动解释为真实零销量。试点中的 353 条按原始正销量区间进一步分为：首次正销量之前 120 条、最后正销量之后 96 条、正销量区间内部 30 条、全期无正销量参照 107 条。前后边界只作为上市/停产或来源覆盖候选，区间内部优先复核，全期无参照车系不直接进入正常历史模型。采集器还将“页面成功但显示—”与“页面根本未取得”分开，后者标记为 `not_retrieved`，不能参与数据结论。

第二批计划覆盖其余 12 个高优先级车系、108 个页面。本轮所有请求均在 TLS 连接层失败，审计状态为 `blocked_no_successful_pages`，因此没有生成新销量、差异或修复；待源站连接恢复后按冻结名单重试。

全量扫描还确认，原先的“至少 24 个连续月份”筛选实际统计的是补齐后的面板行，而不是正销量历史。由于 1,017 个车系都被补成 2022-01 至 2026-06 的 54 行，所有车系在配置合并前都会通过该条件；原始 371 车系中有 2 个全期从未出现正销量，五菱宏光S 修复后剩 1 个，仍有 32 个在训练截止前没有正销量。该条件必须在来源缺口处理完后改成生命周期感知的正销量历史规则，不能现在直接删行重跑。
...
滚动单月主结果中，XGBoost 的全局 WMAPE 为 31.34%，沿用上月销量的朴素基准为 40.99%，相对降低 9.65 个百分点（约 23.5%）。固定六个月压力测试中，最近 6 个月均值为 69.31%，口碑＋冷启动综合方案为 39.07%，相对减少 43.6% 的绝对误差；两者属于不同预测任务。

固定压力测试中的口碑增强点估计改善 0.884 个百分点，5,000 次车系聚类 Bootstrap 的 95% 区间为 −0.284 至 2.108 个百分点，跨过零，因此口碑只作为补充证据。冷启动统计策略处理 9 个历史不足车系，不替代 371 个车系的滚动主模型。
...
完整模型的五折分组交叉验证 R² 为 0.301；年度截面 WMAPE 仅作补充误差，不与月度销量预测 WMAPE 直接比较。
