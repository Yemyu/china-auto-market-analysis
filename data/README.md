...
第四阶段使用 `scripts/42_recrawl_pcauto_sales.py` 按源站唯一 ID 批量回采。每年只请求 6 月、12 月两个半年锚点，原始 HTML 缓存在 Git 忽略目录，采集结果不会覆盖原表。首批 10 个高优先级车系共请求 90 页，全部成功并解析 540 个车系月：186 条与原表完全一致，353 条源站显示“—”且原表同样为零，唯一明确差异为五菱宏光S 2026-06，源站为 2,215、原表为零。该行经车系 ID、页面名称和厂商汇总页复核后加入覆盖层。当前覆盖层共修复 60 行、净恢复 1,357,487 辆；外部队列剩余 21 个高优先级和 33 个中优先级车系。
