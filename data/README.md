...
第二阶段对 9 个“2024 年高销量、2026 测试期全零”车系逐一分型。Model Y 和 Model 3 被确认存在同源快照缺口，已通过覆盖层修复 2026 年 1—6 月共 12 行，恢复销量 238,955 辆；iCAR 03 的测试期零值由同源停售状态支持。其余 6 个车系仍保持零值并进入人工核验，未自动推断或跨车系合并。原始 `monthly_sales.csv` 不被覆盖；修复函数还会校验原值和唯一键，不符合即停止。

Model Y 和 Model 3 的 2024-06 至 2025-12 历史缺口尚未补齐，因此当前覆盖层只用于数据审计，不能直接用于重训或更新最终指标。现有 38.64% 仍是修复前快照上的冻结结果，待完整历史修复后必须重新划分样本并从头评估。

第三阶段对全部 54,918 行、1,017 个车系进行了本地规则扫描。硬结构检查未发现重复车系月、负销量、名称—ID 多对多或缺失月份，但这不等于来源数据正确：年度累计、品牌月销量、品牌有销量车系数、零销量标签和排名是否存在，均与 `monthly_sales` 完全机械一致，不能作为独立交叉验证。风险排序最终保留 57 个待核验车系，其中严重 2 个、高优先级 22 个、中优先级 33 个；人工核验应先处理影响 371 车系模型样本的对象。
