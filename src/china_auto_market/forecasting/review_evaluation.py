...
FIG = PROJECT_ROOT / "assets/analysis"
...
def build_frames(
    *,
    backend: str = "csv",
    login_path: str = "local-auto",
) -> tuple[dict[str, pd.DataFrame], dict[str, list[str]], pd.DataFrame]:
    """Build evaluation frames from frozen files or parity-checked marts."""
    local, review_fixed, review_rolling, local_features, review_features = read_external_tables()
    train, validation, test = mu.load_splits(backend=backend, login_path=login_path)
