...
| 固定六个月压力测试 | 同一 371 个车系 | 一次性从 2026-01 递归预测六个月 | 辅助场景；不能与主结果直接排名 |
...
固定压力测试相对同场景朴素基准减少 43.6% 的绝对误差，但它承受的是六个月递归滞后，和滚动单月不是同一个任务。固定压力测试中的口碑点估计改善 0.884 个百分点；5,000 次车系聚类 Bootstrap 的 95% 区间为 −0.284 至 2.108 个百分点，跨过零，因此只作为补充证据。冷启动统计策略仅处理 9 个历史不足车系，不替代主模型。

销量主模型为 XGBoost；ARIMA、Prophet、LSTM 等早期候选仍保留在实验脚本中，用于记录模型选择过程，不混入当前主结果表。

### 2. 产品配置与年度销量差异

| 特征组合 | 分组交叉验证 R² | 年度截面 WMAPE（补充） |
|---|---:|---:|
| 年份 | 0.089 | 87.77% |
| 年份 + 品牌 | 0.158 | 83.77% |
| 年份 + 品牌 + 配置 | **0.301** | **75.68%** |

加入配置后 R² 比“年份＋品牌”增加约 0.143，说明产品属性能够解释部分车系间年度差异。这里是样本外关联解释，不是因果效应；年度截面 WMAPE 也不与月度销量预测的 WMAPE 直接比较。
