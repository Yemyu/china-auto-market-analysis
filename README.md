...
| 销量基线（XGBoost） | 40.44% | 49.98% | −29.67 pp |
| 用户口碑增强（XGBoost） | 38.71% | 48.83% | −31.41 pp |
| 冷启动补充（统计策略） | **38.64%** | **48.32%** | **−31.47 pp** |
...
销量基线和口碑增强方案使用同一套 XGBoost 主模型，区别仅在于是否加入用户评价信息；冷启动是针对历史不足车系的补充策略，不是另一个主模型。ARIMA、Prophet、LSTM 等早期候选仍保留在实验脚本中。
