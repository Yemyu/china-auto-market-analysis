...
> 数据修复状态：全量规则扫描已覆盖 54,918 行和 1,017 个车系，并将外部核验范围压缩为 57 个对象。同源复核已确认 Model Y 和 Model 3 的原始快照存在缺口，并修复测试期 12 个车系月；两款车的 2024-06 至 2025-12 历史仍待补齐。扫描还发现原“24 个月历史”条件统计的是补齐行，而非正销量历史。因此上表是修复前快照上的冻结基准；完整修复和生命周期样本重建后，将重新训练并重算全部指标。详见[数据修复审计](./data/README.md#数据质量与修复processeddata_quality)。
