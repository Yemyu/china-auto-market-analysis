...
| 方案 | 全局 WMAPE ↓ | 逐车系中位数 WMAPE ↓ | 相对最佳朴素基准 |
|---|---:|---:|---:|
| 最近 6 个月均值（最佳朴素基准） | 70.11% | 90.02% | — |
| 销量基线模型 | 40.44% | 49.98% | −29.67 pp |
| 用户口碑增强模型 | 38.71% | 48.83% | −31.41 pp |
| 冷启动补充方案 | **38.64%** | **48.32%** | **−31.47 pp** |

最终方案相对最佳朴素基准减少 44.9% 的绝对预测误差。用户评价在强销量模型之上带来小幅改善信号，但并未取代历史销量：按车系重采样的 95% 区间为 −0.78 至 5.02 个百分点，仍跨过零。冷启动补充主要改善 9 个历史不足车系，对整体指标的进一步提升为 0.06 个百分点。

> 数据修复状态：全量规则扫描已覆盖 54,918 行和 1,017 个车系；同源回采试点核对了 540 个车系月，替代来源首批又核对了 324 个逐月值。当前覆盖层共修复 62 个车系月、涉及 5 个车系，净恢复销量 1,357,467 辆；其中既有零值缺口，也有 6 个非零错值。替代站用 72 个校准月份取得 100% 一致率后，另由独立来源复核并纠正名爵HS和威马W6各 1 条历史值。两项严重风险已关闭，外部核验队列现剩 21 个高优先级和 33 个中优先级对象。扫描还发现原“24 个月历史”条件统计的是补齐行，而非正销量历史。因此上表仍是修复前快照上的冻结基准；完成高优先级核验和生命周期样本重建后，将重新训练并重算全部指标。详见[数据修复审计](./data/README.md#数据质量与修复processeddata_quality)。
...
38—41  直接多步与朴素基准实验、销量与车系映射全量修复审计
```
