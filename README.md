...
> 数据修复状态：全量规则扫描已覆盖 54,918 行和 1,017 个车系。Model Y 和 Model 3 的同源快照异常已修复 59 个车系月，净恢复销量 1,355,272 辆；其中既有零值缺口，也有 4 个非零错值。两项严重风险已关闭，外部核验队列现剩 22 个高优先级和 33 个中优先级对象。扫描还发现原“24 个月历史”条件统计的是补齐行，而非正销量历史。因此上表仍是修复前快照上的冻结基准；完成高优先级核验和生命周期样本重建后，将重新训练并重算全部指标。详见[数据修复审计](./data/README.md#数据质量与修复processeddata_quality)。
